...
/*   Updated: 2024/01/15 16:13:40 by ottouti          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

static void	ft_plot_line_low(t_point p0, t_point p1, t_mlx_data *data)
{
    ...
    while (coor.x <= p1.x)
    {
        ...
        ft_put_pixel(data, coor.x, coor.y, 0xFF0000);
        if (plt.d > 0)
        {
            coor.y += plt.yi;
            plt.d -= 2 * abs(plt.dx);
...
static void	ft_plot_line_high(t_point p0, t_point p1, t_mlx_data *data)
{
    t_plot_line_tools	plt;
    t_point			coor;
...
static void	ft_plot_line(t_point p0, t_point p1, t_mlx_data *data)
{
	int	dx;
	int	dy;
    ...
    while (i < map->dimensions[0])
    {
        ...
        while (j < map->dimensions[1])
        {
            if (j < map->dimensions[1] - 1)
                ft_plot_line(map->points[i][j], map->points[i][j + 1], data);
            if (i < map->dimensions[0] - 1)
                ft_plot_line(map->points[i][j], map->points[i + 1][j], data);
            j++;
        }
        i++;
    }
